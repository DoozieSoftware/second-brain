...
- Name: tests/ui-test.spec.ts >> Second Brain UI Tests >> should show savings alerts after scan
- Location: tests/ui-test.spec.ts:107:3
...
Call log:
  ...
  - waiting for locator('button:has-text("Find Savings")')
    6 × locator resolved to <button class="btn" onclick="scanAlerts()">…</button>
      - unexpected value "
        🔍 Find Savings
        Scan for waste & duplicates
      "
